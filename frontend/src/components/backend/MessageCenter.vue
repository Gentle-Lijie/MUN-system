...
const formError = ref('')
const sendForm = reactive<{ target: MessageTarget; targetId: number | null; committeeId: number | null; content: string }>(
    {
        target: 'everyone',
        targetId: null,
        committeeId: null,
    ...
    <div class="p-6 space-y-6">
        <header
            ...
            <div>
                ...
                <p class="text-sm text-base-content/70">{{ props.subtitle || '查看历史消息并推送公告。' }}</p>
            </div>
            <div class="flex flex-wrap gap-2">
                <button class="btn btn-outline btn-sm" @click="fetchMessages" :disabled="loading">刷新列表</button>
                <button class="btn btn-primary btn-sm" :disabled="!allowedTargets.length" @click="openSendModal">
                    发送消息
                </button>
            </div>
        </header>
        ...
        <section class="rounded-2xl border border-base-200 bg-base-100 p-4 space-y-4">
            <div class="grid gap-3 md:grid-cols-4">
                ...
                <FormField legend="所属会场" label="限定可选会场">
                    <select v-model.number="filters.committeeId" class="select select-bordered select-sm"
                        ...
                        <option v-for="committee in recipients.committees" :key="committee.id" :value="committee.id">
                    ...
                    <tbody>
                        ...
                        <tr v-for="message in messages" :key="message.id">
                            ...
                            <td class="align-top">
                                ...
                                <div class="text-xs text-base-content/60">{{ message.sender?.role || 'system' }}</div>
                            </td>
                            <td class="align-top">
            ...
            <div class="flex items-center justify-between text-sm text-base-content/70">
                ...
                <div class="join">
                    <button class="btn btn-sm join-item" :disabled="!canPrev" @click="changePage(-1)">上一页</button>
                    <button class="btn btn-sm join-item" :disabled="!canNext" @click="changePage(1)">下一页</button>
                </div>
            </div>
        </section>
                ...
                <div class="grid gap-3">
                    ...
                    <FormField v-if="sendForm.target === 'committee' || sendForm.target === 'dias'" legend="选择会场"
                        ...
                        <select class="select select-bordered" :value="sendForm.targetId ?? 0"
                            ...
                            <option v-for="committee in committeeOptions" :key="committee.id" :value="committee.id">
                                {{ committee.name }} ({{ committee.code }})
                            </option>
                        </select>
                    </FormField>
                    ...
                    <FormField v-if="sendForm.target === 'delegate'" legend="选择代表" label="指定某位代表">
                        <select class="select select-bordered" :value="sendForm.targetId ?? 0"
                            ...
                            <option v-for="delegate in delegateOptions" :key="delegate.userId" :value="delegate.userId">
