...
function showNewMessageNotification(messageOrCount: MessageRecord | number) {
  ...
  const hasPermission =
    (typeof notificationPermission.value !== 'undefined' &&
      notificationPermission.value === 'granted') ||
  ...
  try {
    ...
    notification.onclick = () => {
      ...
      if (typeof activeTab.value !== 'undefined') {
...
function testNotification(count = 1) {
  ...
  if (
    typeof notificationPermission.value === 'undefined' ||
    notificationPermission.value === undefined
  ) {
...
async function fetchMessages() {
  ...
  try {
    ...
    const hasNewMessage =
      lastMessageContent.value && currentMessageContent !== lastMessageContent.value
      ...
      <div class="card-body flex flex-col gap-8">
        ...
        <div v-if="activeTab === 'files'" class="flex flex-col gap-6">
          ...
          <button
            ...
            :disabled="submitting || !fileForm.title || !fileForm.file"
            @click="submitFile"
          >
        ...
        <div v-else-if="activeTab === 'messages'" class="flex flex-col gap-6">
          ...
          <button
            class="btn btn-primary w-full"
            :disabled="submitting || !messageForm.content"
            @click="sendMessage"
          >
            <span v-if="submitting" class="loading loading-spinner loading-sm"></span>
            发送
        ...
        <div v-else-if="activeTab === 'messageList'" class="flex flex-col gap-4">
          ...
          <div class="flex justify-between items-center">
            <span class="text-sm text-base-content/70"
              >共 {{ messagePagination.total }} 条消息</span
            >
            <button
              ...
              >
                <div class="flex justify-between items-start mb-2">
                  ...
                  <div class="badge badge-outline badge-xs">
                    {{ targetLabelMap[message.target] }}
                  </div>
                </div>
                <div class="text-sm font-medium mb-1">
                  {{ message.sender?.name || '系统' }}
                  <span class="text-xs text-base-content/60"
                    >· {{ message.sender?.role || 'system' }}</span
                  >
                </div>
          ...
          >
            ...
            <button
              ...
              :disabled="
                messagePagination.page >=
                Math.ceil(messagePagination.total / messagePagination.pageSize)
              "
              @click="changeMessagePage(1)"
            >
              下一页
