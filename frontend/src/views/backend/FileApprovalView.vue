        ...
        <article v-for="file in submissions" :key="file.id"
          ...
          <div class="flex justify-between items-start">
            ...
            <div class="flex gap-2">
              <a :href="`${API_BASE}${file.content_path}`" target="_blank" class="btn btn-xs btn-outline">查看文件</a>
              <span class="badge badge-warning">待审批</span>
            </div>
          </div>
          <p class="text-sm">{{ file.description }}</p>
          <p class="text-xs text-base-content/50">
            提交时间：{{ file.submitted_at ? new Date(file.submitted_at).toLocaleString() : '未知' }}
          </p>
        </article>
        <div v-if="submissions.length === 0 && !loading" class="alert alert-info">暂无待审批文件</div>
      </div>
