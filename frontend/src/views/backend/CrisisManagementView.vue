...
const targetLabel = (ids: number[] | null) => {
  ...
  return ids.map((id) => committeeLookup.value[id]?.name || `#${id}`)
}
  ...
  <div class="p-6 space-y-6">
    <header class="border-b border-base-200 pb-4 flex flex-wrap items-center gap-4">
      ...
      <div class="ml-auto flex flex-wrap gap-3 items-center">
        ...
        <button class="btn btn-sm" :disabled="loading" @click="fetchCrises">刷新</button>
      </div>
    </header>
    ...
    <section class="grid gap-6 xl:grid-cols-[1.6fr,1fr]">
      <div class="space-y-4">
        ...
        <p v-else-if="filteredCrises.length === 0" class="text-center text-base-content/60 py-10">
          暂无危机记录
        </p>
      ...
      <form class="border border-base-200 rounded-2xl space-y-4" @submit.prevent="saveCrisis">
        <div class="flex items-center justify-between">
          ...
          <button v-if="editingId" type="button" class="btn btn-xs btn-ghost" @click="resetForm">
            新建
          </button>
        </div>
