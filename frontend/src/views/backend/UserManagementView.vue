...
const handleImportFile = async (event: Event) => {
    ...
    try {
        ...
        const errors = Array.isArray(result?.errors) && result.errors.length > 0 ? `，其中 ${result.errors.length} 条记录未导入` : ''
...
const handleExport = async () => {
    ...
    try {
        ...
        link.href = url
        link.download = response.headers.get('Content-Disposition')?.split('filename=')[1]?.replace(/"/g, '') || 'users.csv'
...
<template>
    <div class="p-6 space-y-6">
        <header class="border-b border-base-200 pb-4">
            ...
            <p class="text-base-content/70 text-sm">实时对接后台，支持搜索、导入/导出以及管理员重置密码等操作。</p>
        </header>
                ...
                <div class="overflow-x-auto border border-base-200 rounded-xl">
                    <table class="table table-zebra">
                        ...
                        <tbody>
                            ...
                            <tr v-else v-for="user in filteredUsers" :key="user.id" @click="handleRowClick(user)"
                                class="cursor-pointer" :class="selectedUser?.id === user.id ? 'bg-primary/10' : ''">
                                <td>
                                ...
                                <td>
                                    <span class="badge"
                                        :class="{ 'badge-secondary': user.role === 'admin', 'badge-primary': user.role === 'dais', 'badge-accent': user.role === 'delegate', 'badge-neutral': user.role === 'observer' }">
            ...
            <div class="space-y-6">
                <div class="border border-base-200 rounded-xl p-4 bg-base-200/50" v-if="selectedUser">
                    ...
                    <div class="space-y-2 text-sm">
                        ...
                        <p><span class="text-base-content/60">角色：</span>{{ roleLabelMap[selectedUser.role] }}</p>
                        <p><span class="text-base-content/60">学校：</span>{{ selectedUser.organization || '未填写' }}
                        </p>
                        <p><span class="text-base-content/60">联系电话：</span>{{ selectedUser.phone || '未填写' }}</p>
                        <p><span class="text-base-content/60">最近登录：</span>{{ selectedUser.lastLogin || '尚未登录' }}</p>
                        <p><span class="text-base-content/60">创建时间：</span>{{ selectedUser.createdAt || '-' }}</p>
                    </div>
                    <div class="mt-4 flex gap-2">
                        <button class="btn btn-primary btn-sm" :disabled="resetting" @click="handleResetPassword">
