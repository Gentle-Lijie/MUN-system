...
<template>
    <section class="min-h-screen bg-base-200 p-6 lg:p-10">
        <div class="mx-auto flex max-w-6xl flex-col gap-6 rounded-3xl bg-base-100 p-6 lg:p-10 shadow-xl">
            <header class="flex flex-wrap items-center gap-4 border-b border-base-200 pb-4">
                <div>
                    ...
                    <p class="text-base text-base-content/60">实时读取数据库中的参考文件，便于动议与文件互相绑定。</p>
                </div>
                <div class="ml-auto flex flex-wrap gap-3">
                    <button class="btn btn-primary" @click="showFileSelect = true">打开文件弹窗</button>
                    <button class="btn btn-ghost" @click="fetchReferences" :disabled="loading">
                        {{ loading ? '加载中...' : '刷新列表' }}
                    </button>
                    ...
                    <div class="flex flex-wrap items-center gap-3">
                        ...
                        <div class="grow">
                            <label class="input input-bordered flex items-center gap-2">
                                ...
                                <input type="text" v-model="search" placeholder="按标题/类型/会场搜索" class="grow" />
                            </label>
                        </div>
                    </div>
                    <div class="rounded-2xl border border-dashed border-base-300 min-h-[260px] overflow-hidden">
                        <div v-if="loading" class="flex items-center justify-center py-16">
                            <span class="loading loading-spinner loading-lg"></span>
                        </div>
                        <div v-else-if="filteredFiles.length === 0" class="text-center text-base-content/60 py-16">
                            暂无匹配的文件，请尝试取消筛选或等待管理员发布。
                        </div>
                        <div v-else class="overflow-y-auto max-h-[360px] divide-y divide-base-200">
                            <button v-for="file in filteredFiles" :key="file.id" type="button"
